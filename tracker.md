...
- 2026-09-22 — Admin sync pass: every remaining hard-coded string is now an editable field. Page text gained four tabs — **City pages** (all section labels plus the FAQ list shown on all 24 city pages, with `{city}` / `{arrival}` placeholders; a question needing `{arrival}` stays hidden until that verified figure exists), **Service pages** (included/areas/related labels, Bay Area page headings), **About page** (standards heading + the three cards, service-area block, dispatch-hub heading) and **Empty states** (pricing and reviews wording). Home page gained the About block (heading, paragraph, three points, button) and the FAQ bar line. `npm run copy:defaults` fills blank fields on an existing site without touching edited text — run after deploying new fields. Verified by editing 8 fields through the API and seeing each change on the live page. Dashboard's launch panel is readable in dark mode. Live site re-checked: 49 pages, 0 SEO problems, TTFB ~0.2s.
